...
import type { SponsoredAddonsShelfType } from 'amo/reducers/home';
...
type Props = {|
  ...
  shelfData: SponsoredAddonsShelfType | null,
  className?: string,
  loading: boolean,
|};
...
export class PromotedAddonsCardBase extends React.Component<InternalProps> {
  ...
  onAddonClick = (addon: AddonType | CollectionAddonType) => {
    ...
    if (_config.get('enableFeatureUseAdzerk') && addon && addon.clickURL) {
      sendBeacon({
        _navigator,
        data: addon.clickData,
        urlString: addon.clickURL,
      });
    }
  ...
  onShelfImpression = () => {
    ...
    if (
      _config.get('enableFeatureUseAdzerk') &&
      shelfData &&
      shelfData.impressionURL
    ) {
      sendBeacon({
        _navigator,
        data: shelfData.impressionData,
        urlString: shelfData.impressionURL,
      });
    }
  };
